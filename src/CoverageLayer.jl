...
"""
function _base_driver_cmd(;
    runner_dir::AbstractString="scripts/TestimonialRunner",
    artifact_dir::Union{String,Nothing}=nothing,
)::Vector{String}
...
function build_driver_command(
    ...
    cmd = _base_driver_cmd(; runner_dir=runner_dir, artifact_dir=artifact_dir)
    env = Dict(
        "TESTIMONIAL_FILE" => String(test_file),
        "TESTIMONIAL_ITEM" => String(item_name),
    )
    if artifact_dir !== nothing
        env["TESTIMONIAL_TRACE_OUTPUT_DIR"] = String(artifact_dir)
    end
    return (cmd, env)
end
...
"""
    _find_tracefile(; artifact_dir=nothing) -> Union{String, Nothing}

Search for the LCOV tracefile generated by Julia 1.12+ `--code-coverage=tracefile.info`.

When an `artifact_dir` is provided (testimonial-in3s.2), look there first
for the isolated tracefile. Otherwise, fall back to the default pwd check.
"""
function _find_tracefile(; artifact_dir::Union{String,Nothing}=nothing)
    # When artifact isolation is active, read from the isolated directory
    if artifact_dir !== nothing
    ...
    # Fallback: check pwd (legacy behavior for un-isolated recordings)
...
function record_item(runner::SubprocessRunner, ref)
    ...
    # Create a unique per-attempt artifact directory so that coverage
    # artifacts (tracefile.info, inference_trace.jls) are isolated
    # from other concurrent subprocesses (testimonial-in3s.2).
    artifact_dir = mktempdir()
    try
        # Build the subprocess command with isolated artifact paths
        cmd, env = build_driver_command(test_file, ref.name; runner_dir=runner.runner_dir, artifact_dir=artifact_dir)
        ...
        # Parse coverage from the isolated artifact directory
        test_covered, test_uncovered, source_files = _collect_coverage(test_file, parent; artifact_dir=artifact_dir)

        return parent.ItemCoverage(ref, test_covered, test_uncovered, source_files)
    finally
        # Clean up the artifact directory
        try rm(artifact_dir; recursive=true, force=true) catch end
    end
end
...
function record_batch(runner::SubprocessRunner, refs::Vector)
    ...
    # Create a unique per-attempt artifact directory
    artifact_dir = mktempdir()
    try
        cmd, env = build_driver_command(test_file, names; runner_dir=runner.runner_dir, artifact_dir=artifact_dir)
...
function record_file(runner::AbstractRunner, test_file::AbstractString)
    ...
    artifact_dir = mktempdir()
    try
        cmd, env = build_driver_command(test_file; runner_dir=runner.runner_dir, artifact_dir=artifact_dir)
...
"""
function _collect_coverage(test_file::String, parent::Module; artifact_dir::Union{String,Nothing}=nothing)
    covered_lines = Int[]
    uncovered_lines = Int[]
